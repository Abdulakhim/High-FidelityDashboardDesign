...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      watch: { ignored: ['**/.figma/**'] },
      // When running behind the Figma proxy the browser reaches the server
      // through HTTPS/443, not the internal dev port. Tell the HMR client
      // to connect on port 443 so the WebSocket goes through the same proxy.
